...
import picContent from "../components/renderPic.js";
import renderLikesPictures from "../components/renderLikes.js";
import { likesIdArr } from "../components/renderCards.js";
...
/********* Picture Info **********/
export const getIdInArr = (idArr, pictures) => { 
  idArr.forEach((id) => {
		document
			.getElementById(id)
			.addEventListener('click', () => picContent(id, pictures));
	});
};

/********* Like Picture **********/
export const clickLikeBtn = (likesIdArr, pictures) => {
	likesIdArr.forEach((id) => {
		document
			.getElementById(id)
			.addEventListener('click', () => renderLikesPictures(id, pictures));
	});
};

/********* XXXXXXXXXXXXXXXXXXXXXXX **********/
export const toDeleteLike = (arrAfterLikes) => {
  arrAfterLikes.forEach((id) => {
    picToDelete = document
			.getElementById(id)
      .addEventListener('click', () => {
        let temp = arrAfterLikes.filter((likeId) => likeId !== id);
        renderLikesPictures(id, temp);
      })
	});
}

/********* XXXXXXXXXXXXXXXXXXXXXXX **********/
// export const deletePicFromLikes = (id, pictures) => {
 
  
// };

// let counter = 0;
  // arrAfterLikes.find((item) => {
  //   if (item === id && counter === 1) {
  //     throw new Error('error in deletePicFromLikes func');
  //   } else if (item === id) {
  //     counter++;
  //   }
  // }, counter = 0);
	// console.log('Before filter: ' + arrAfterLikes);
	// const temp = arrAfterLikes.filter((like) => like !== id);
	// console.log('After filter: ' + temp);
	// clickLikeBtn(temp, pictures);




/********* XXXXXXXXXXXXXXXXXXXXXXX **********/
// export const checkIfLikeClicked = (picId) => {
//   beforeLike.push(picId)
//   return beforeLike.includes(picId);
  
// }

  // let counter = 0;
  // if (!beforeLike.length) {
  //   beforeLike.push(picId);
  //   return true;
  // }
  // // console.log(picId);
  // for (let i = 0; i < beforeLike.length; i++) {
  //   // console.log(beforeLike[i]);
  //   if (beforeLike[i] === picId && counter === 1) {
  //     // console.log(counter);
  //     return false;
  //   } else if (beforeLike[i] === picId) {
  //     console.log(counter);
  //     counter++;
  //     console.log(counter);
	// 	}
  ...
  // טיפול בהוספת תמונה ללייקים ומחיקתם